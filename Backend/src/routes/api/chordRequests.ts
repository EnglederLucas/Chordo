...
router.post("/chords", (req: any, res: any) => {
	// console.log(req.body.link);
	var fileName = null;
	console.log(req.body.link);

	scraperUG.get(req.body.link, (error: any, chords: any) => {
...
router.post("/chords_raw", (req: any, res: any) => {
	scraperUG.get(req.body.link, (error: any, chords: any) => {
		if (error) console.log(error);
		else {
...
router.get("/downloadChords/:id", (req: any, res: any) => {
	console.log("download" + req.body.fileName);
	res.download(req.body.fileName.toString(), "chords.pdf");
});

function processText(chordPage: any, options: any) {
